...
設定画面の「暗号化バックアップ」から、TOTPシード、大学アカウント、自動化設定をJSONファイルへ書き出せます。現在の保護方式にかかわらず、バックアップ用パスワードからPBKDF2-SHA256で鍵を導出し、AES-GCMで再暗号化します。

別の端末またはブラウザープロファイルでは、設定画面の「暗号化バックアップをインポート」からファイルとバックアップ用パスワードを指定してください。インポート後は、バックアップ用パスワードが新しいマスターパスワードになります。

> [!CAUTION]
> バックアップファイルには暗号化されたTOTPシードと大学アカウント情報が含まれます。バックアップ用パスワードと同じ場所へ保存したり、第三者へ送信したりしないでください。

## 画面変更の診断

ADFSページで既知のログイン画面、MFA選択画面、TOTP入力画面のいずれも認識できない状態が4秒以上続くと、画面右下に診断パネルを表示します。

「診断情報をコピー」を押すと、拡張機能のバージョン、URLのパス、検出した要素IDなどをコピーできます。入力値、TOTPシード、大学パスワード、URLのクエリ文字列は含みません。
...
マスターパスワード、バックアップ用パスワード、TOTPシード、大学アカウント情報をソースコードやIssueへ記載しないでください。
